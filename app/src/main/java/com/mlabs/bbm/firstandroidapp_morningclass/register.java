...
public class register extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            myDB.execSQL("CREATE TABLE IF NOT EXISTS Register (Username VARCHAR, Email VARCHAR, Password VARCHAR,First Name VARCHAR, Last Name VARCHAR);");
        }finally {
        ...
        switch (v.getId()) {
            case R.id.button:
                ...
                if(email.length()>1 && user.length()>1 && pass.length()>1 && cpass.length()>1&& fname.length()>1&& lname.length()>1){
                    if(pass.equals(cpass)){
                        if(email.trim().matches(emailPattern) || email.trim().matches(emailPattern2)){
                            if(pass.length()>8 && cpass.length()>8){
                                if(user.length()>8){
                                    ...
                                    if(cursor.getCount()<1){
                                        ...
                                        if(cursor1.getCount()<1){
                                            ...
                                            newValues.put("First Name",fname);
                                            newValues.put("Last Name",lname);
                                ...
                                }else Toast.makeText(getBaseContext(), "Username too short", Toast.LENGTH_SHORT).show();
                    ...
                    }else Toast.makeText(getBaseContext(), "Password doesn't match", Toast.LENGTH_SHORT).show();
